...
def build_data_cv(data_folder,cv = 3,clean_string= True):
    """
    Loads data and split into 10 folds.
    """
    revs = []
    intro_file = data_folder[0]
    close_file = data_folder[1]
    wait_file = data_folder[2]
    vocab = defaultdict(float)
    with codecs.open(intro_file, "rb",'utf_8') as f:
        for line in f: 
            rev = []
            rev.append(line)
            ...
            datum  = {"y":0, 
                      ...
                      "split": np.random.randint(0,cv)}
            revs.append(datum)
    with codecs.open(close_file, "rb",'utf_8') as f:
        for line in f: 
            rev = []
            rev.append(line)
            ...
            datum  = {"y":1, 
                      ...
                      "split": np.random.randint(0,cv)}
            revs.append(datum)
    with codecs.open(wait_file, "rb",'utf_8') as f:
        for line in f: 
            rev = []
            rev.append(line)
        ...
        for row in tsvin:
            ...
            if (len(row[0])<=6):
                ...
                if read_flag:
                    ...
                    a = re.sub('\[','',row[2])
                    last_vec = np.append(last_vec, map(np.float32, a.split()))
                else:
                    continue
...
if __name__=="__main__":    
    w2v_file =  'zh/zh.tsv'   
    data_folder = '/Users/wangwei/cuda_keras_projets/clevo/CNN_clevo/chen_test'#["textFiles/text.S_INTRO", "textFiles/text.S_END","textFiles/text.S_WAIT_WARNING"]     
    print("loading data...")        
